...
describe('keyword-detector.mjs mode-message dispatch', () => {
    ...
    it('does not activate ultragoal for a descriptive "when i use ultragoal" mention', () => {
        ...
        const output = runKeywordDetector('sometimes when i use ultragoal to create durable goals, at the end it tells me the /goal command to run like: /goal Complete all 16 ultragoal stories. But often it does not.', cwd, sessionId);
        const context = output.hookSpecificOutput?.additionalContext ?? '';
