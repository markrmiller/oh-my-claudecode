...
// CHANGE 3: project roots that may carry OMC skill/agent overrides.
...
async function main() {
  ...
  try {
    ...
    // ultrawork, ultragoal, ralplan) to auto-activate via an explicit,
    // prompt-leading slash invocation. Prevents phantom modes from pasted logs.
    ...
    // Ralph keywords
    if (allowStateModeActivation('ralph') && hasActionableKeyword(cleanPrompt, /\b(ralph|don't stop|must complete|until done)\b|(랄프)(?!로렌)|(ラルフ)(?!・?ローレン)/i)) {
    ...
    if (allowStateModeActivation('autopilot') && (hasActionableKeyword(cleanPrompt, /\b(autopilot|auto pilot|auto-pilot|full auto|fullsend)\b|(오토파일럿)|(オートパイロット)/i) ||
    ...
    // Ultragoal keywords
    if (allowStateModeActivation('ultragoal') && hasExplicitActionableKeyword(cleanPrompt, /\b(ultragoal)\b/i)) {
    ...
    // Ultrawork keywords
    if (allowStateModeActivation('ultrawork') && hasActionableKeyword(cleanPrompt, /\b(ultrawork|ulw|uw)\b|(울트라워크)|(ウルトラワーク)/i)) {
